...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    public void DrawOptionBar()
    {
		...
		if (GUILayout.Button("Force Update", toolbarButtonGUIStyle))
		{
			data.ClearCache();
			data.CollectDependenciesInfo();
		}
		...
		if (GUILayout.Button("Expand First Depth", toolbarButtonGUIStyle)) {
			foreach (var item in m_AssetTreeView.assetRoot.children) {
				m_AssetTreeView.SetExpanded(item.id, true);
			}
		}
        //扩展
