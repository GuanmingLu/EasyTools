...
void DoDebug_float(float4 val, int decimalDigits, float2 uv, float2 textSize, out float4 res)
{
    float2 posDelta = float2(0, textSize.y);
    float fMaxDigits = -1;
    res =
    PrintValue(uv, posDelta*2.5, textSize, val.x, fMaxDigits, decimalDigits) * float4(1,0,0,1) +
    PrintValue(uv, posDelta*0.5, textSize, val.y, fMaxDigits, decimalDigits) * float4(0,1,0,1) +
    PrintValue(uv, posDelta*-1.5, textSize, val.z, fMaxDigits, decimalDigits) * float4(0,0,1,1) +
    PrintValue(uv, posDelta*-3.5, textSize, val.w, fMaxDigits, decimalDigits);
}
